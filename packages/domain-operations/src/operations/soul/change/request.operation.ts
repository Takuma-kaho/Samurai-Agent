...
export interface SoulChangeRequestPorts extends HumanChangeRequestPorts {}
...
const soulChangeRequest = defineCommand<SoulChangeRequestPorts>()({
  ...
  resourceKinds: ["activity", "soul"],
  proposedEffects: ["Record a human review request without changing Soul."],
  ...
  provenance: [{
    ...
    decision: "adapted",
    reason: "Human-owned Soul may be requested by an external Client but is never changed by it directly."
  }],
  input: Input,
  output: Output,
  createHandler(ports) {
    return {
      execute: async function handleSoulChangeRequest(context: TrustedDomainContext, input): Promise<DomainResult<HumanChangeRequestOutput>> {
        const value = await ports.requestHumanChange(context, { ...input, request_kind: "soul" });
...
export default soulChangeRequest;
