...
export interface SkillSearchPorts extends DomainQueryPorts {
  searchSkills: ReadCapability<(query: string, limit: number) => Promise<z.infer<typeof Output>>>;
}
...
const skillSearch = defineQuery<SkillSearchPorts>()({
  ...
  createHandler(ports) {
    return {
      execute: async function handleSkillSearch(_context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> {
        return { ok: true, value: Output.parse(await ports.searchSkills(input.query, input.limit)) };
      }
    };
  }
});
