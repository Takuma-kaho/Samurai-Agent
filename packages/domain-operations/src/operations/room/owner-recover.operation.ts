...
const Input = z.object({ room_id: z.string().trim().min(1), owner_participant_id: humanParticipantIdSchema }).strict();
...
const roomOwnerRecover = defineCommand<RoomOwnerRecoverPorts>()({
  ...
  createHandler(ports) { return { execute: async function handleRoomOwnerRecover(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.recoverOwnerlessRoom(context, { roomId: input.room_id, ownerParticipantId: input.owner_participant_id })) }; } }; }
});
