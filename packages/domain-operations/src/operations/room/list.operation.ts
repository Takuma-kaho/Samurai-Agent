...
export interface RoomListPorts extends DomainQueryPorts { listRooms: ReadCapability<(context: TrustedDomainContext) => Promise<z.infer<typeof Output>>>; }
const roomList = defineQuery<RoomListPorts>()({
  ...
  createHandler(ports) { return { execute: async function handleRoomList(context: TrustedDomainContext, _input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.listRooms(context)) }; } }; }
});
