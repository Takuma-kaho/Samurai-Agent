...
const Input = z.object({ source_room_id: z.string().trim().min(1), resource: roomShareableResourceReferenceSchema }).strict();
...
const roomResourceShareList = defineQuery<RoomResourceShareListPorts>()({
  ...
  createHandler(ports) { return { execute: async function handleRoomResourceShareList(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.listResourceShares(context, { sourceRoomId: input.source_room_id, resource: shareReference(input.resource) })) }; } }; }
});
