...
const Input = z.object({ source_room_id: z.string().trim().min(1), target_room_id: z.string().trim().min(1), resource: roomShareableResourceReferenceSchema }).strict();
...
const roomResourceShareRevoke = defineCommand<RoomResourceShareRevokePorts>()({
  ...
  createHandler(ports) { return { execute: async function handleRoomResourceShareRevoke(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.revokeResourceShare(context, { sourceRoomId: input.source_room_id, targetRoomId: input.target_room_id, resource: shareReference(input.resource) })) }; } }; }
});
