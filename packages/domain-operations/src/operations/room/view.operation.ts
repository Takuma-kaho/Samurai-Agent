...
export interface RoomViewPorts extends DomainQueryPorts { viewRoom: ReadCapability<(context: TrustedDomainContext, id: string) => Promise<z.infer<typeof Output>>>; }
const roomView = defineQuery<RoomViewPorts>()({
  ...
  sources: ["runtime_api", "external_app"], render: ["status_timeline"], resourceKinds: ["room"], proposedEffects: ["Read a Room."], outputResourceKind: "room", uiDisplayCategory: "workspace",
  ...
  createHandler(ports) { return { execute: async function handleRoomView(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.viewRoom(context, input.id)) }; } }; }
});
