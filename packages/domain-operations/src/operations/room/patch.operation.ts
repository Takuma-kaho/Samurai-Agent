...
const Input = z.object({ id: z.string().trim().min(1), name: z.string().trim().min(1).max(200) }).strict();
const Output = roomValueSchema;
export interface RoomPatchPorts { patchRoom(input: z.infer<typeof Input>): Promise<z.infer<typeof Output>>; }
const roomPatch = defineCommand<RoomPatchPorts>()({
  ...
  createHandler(ports) { return { execute: async function handleRoomPatch(_context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.patchRoom(input)) }; } }; }
});
