...
const roomMemberRoleChange = defineCommand<RoomMemberRoleChangePorts>()({
  id: "room.member.role.change", version: "1.1", availability: "active", title: "Change Room participant role", description: "Change a non-owner human Room role.",
  ...
  provenance: [{ source: "samurai", commit_sha: "core-06", reference_file: "ARCHITECTURE.md", decision: "adapted", reason: "Owner transfer has a separate atomic operation." }], input: Input, output: Output,
  createHandler(ports) { return { execute: async function handleRoomMemberRoleChange(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.changeRoomMemberRole(context, { roomId: requireRoomContext(context, "room.member.role.change"), participantId: input.target_participant_id, role: input.role })) }; } }; }
});
