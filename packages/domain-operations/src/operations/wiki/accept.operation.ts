  ...
  createHandler(ports) {
    return {
      execute: async function handleWikiAccept(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> {
        return { ok: true, value: await executeWikiStateTransition(ports, { context, id: input.wiki_id, state: "active", expectedResourceVersion: input.expected_resource_version, operationName: "wiki.accept", proposedEffect: "Accept a wiki proposal for active retrieval.", summaryPrefix: "Accepted wiki page" }) };
      }
    };
  }
});
