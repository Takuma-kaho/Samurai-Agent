...
export interface AutomationJobRunPorts {
  runSessionlessAutomationJob(input: { jobId: string; now: string }): Promise<z.infer<typeof Output>>;
}
  ...
  createHandler(ports) {
    return {
      execute: async function handleAutomationJobRun(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> {
        ...
        return { ok: true, value: Output.parse(await ports.runSessionlessAutomationJob({ jobId: input.job_id, now })) };
      }
    };
  }
});
