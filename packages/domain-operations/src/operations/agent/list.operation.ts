...
export interface AgentListPorts extends DomainQueryPorts { listAgents: ReadCapability<() => Promise<z.infer<typeof Output>>>; }
const agentList = defineQuery<AgentListPorts>()({
  ...
  createHandler(ports) { return { execute: async function handleAgentList(_context: TrustedDomainContext, _input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.listAgents()) }; } }; }
});
