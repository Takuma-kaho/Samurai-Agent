...
export interface AgentViewPorts extends DomainQueryPorts { viewAgent: ReadCapability<(id: string) => Promise<z.infer<typeof Output>>>; }
const agentView = defineQuery<AgentViewPorts>()({
  ...
  createHandler(ports) { return { execute: async function handleAgentView(_context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.viewAgent(input.id)) }; } }; }
});
