...
export interface AgentCreatePorts { createAgent(context: TrustedDomainContext, input: { name: string; role: string; instructions: string; backendId: string; enabled?: boolean }): Promise<z.infer<typeof Output>>; }
const agentCreate = defineCommand<AgentCreatePorts>()({
  ...
  createHandler(ports) { return { execute: async function handleAgentCreate(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.createAgent(context, { name: input.name, role: input.role, instructions: input.instructions, backendId: input.backend_id, enabled: input.enabled })) }; } }; }
});
