...
const Input = z.object({ id: z.string().trim().min(1), name: z.string().trim().min(1).max(200).optional(), role: z.string().trim().min(1).max(500).optional(), instructions: z.string().trim().min(1).max(20_000).optional(), enabled: z.boolean().optional(), expected_version: z.number().int().positive().optional() }).strict().refine((value) => value.name !== undefined || value.role !== undefined || value.instructions !== undefined || value.enabled !== undefined, "agent_patch_empty");
const Output = agentValueSchema;
export interface AgentPatchPorts { patchAgent(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<z.infer<typeof Output>>; }
const agentPatch = defineCommand<AgentPatchPorts>()({
  id: "agent.patch", version: "1.1", availability: "active", title: "Update Agent", description: "Update Agent identity, role, instructions, or enabled state.",
  ...
  createHandler(ports) { return { execute: async function handleAgentPatch(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.patchAgent(context, input)) }; } }; }
});
