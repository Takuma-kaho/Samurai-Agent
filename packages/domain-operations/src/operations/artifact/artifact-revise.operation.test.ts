...
describe("artifact.revise handler", () => {
  it("owns session selection, source mapping, revision creation, and rollback", async () => {
    ...
    const handler = artifactRevise.createHandler({
      ...
      artifactNotFoundError: () => new Error("artifact_not_found"), getArtifactSession: async () => session,
      ensureArtifactSession: async () => session, artifactSessionNotFoundError: () => new Error("session_not_found"),
      validateGraphArtifactContent: () => undefined, createArtifactEnvelope: () => envelope,
    ...
    const result = await handler.execute(context, artifactRevise.input.parse({ artifact_id: artifact.id, content: "New body", session_id: session.id }));

    expect(createArtifactRevision).toHaveBeenCalledWith(expect.objectContaining({ artifactId: artifact.id, content: "New body", editorSource: "provider" }));
  ...
  it("validates graph content before starting a mutation", async () => {
    ...
    const handler = artifactRevise.createHandler({
      ...
      artifactNotFoundError: () => new Error("artifact_not_found"), getArtifactSession: async () => session,
      ensureArtifactSession: async () => session, artifactSessionNotFoundError: () => new Error("session_not_found"),
      validateGraphArtifactContent, createArtifactEnvelope: () => envelope,
