...
const contextWithSession: TrustedDomainContext = { ...context, sessionId: "session_1", runId: "run_1" };
...
describe("artifact.revise handler", () => {
  it("owns session selection, source mapping, revision creation, and rollback", async () => {
    ...
    const result = await handler.execute(contextWithSession, artifactRevise.input.parse({ artifact_id: artifact.id, content: "New body", editor_source: "provider" }));
