...
export interface BrowserScreenshotPorts {
  executeBrowserScreenshot(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> | DomainResult<z.infer<typeof Output>>;
}
  ...
  createHandler(ports) {
    return {
      execute: async function handleBrowserScreenshot(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> {
        return ports.executeBrowserScreenshot(context, input);
      }
    };
  }
});
