...
export interface BrowserNavigatePorts {
  executeBrowserNavigate(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> | DomainResult<z.infer<typeof Output>>;
}
  ...
  createHandler(ports) {
    return {
      execute: async function handleBrowserNavigate(context: TrustedDomainContext, input: z.infer<typeof Input>): Promise<DomainResult<z.infer<typeof Output>>> {
        return ports.executeBrowserNavigate(context, input);
      }
    };
  }
});
