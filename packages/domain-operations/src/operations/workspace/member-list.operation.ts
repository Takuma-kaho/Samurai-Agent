...
const workspaceMemberList = defineQuery<WorkspaceMemberListPorts>()({
  ...
  createHandler(ports) { return { execute: async function handleWorkspaceMemberList(context: TrustedDomainContext): Promise<DomainResult<z.infer<typeof Output>>> { return { ok: true, value: Output.parse(await ports.listWorkspaceMembers(context)) }; } }; }
});
