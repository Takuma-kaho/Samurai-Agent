...
import { bindOperationDefinitions, jsonSchemaFor, operationDefinitions, type DomainOperationPorts } from "./index.js";
...
describe("Domain Operation strict gate coverage", () => {
  ...
  it("executes every real operation handler through only its operation-specific Port", async () => {
      ...
      get: (_target, operationId) => new Proxy({}, {
        get: (_ports, method) => (...args: unknown[]) => {
          ...
          if (name === "getGeneratedSurface") {
            ...
            return { ...surface, id: "sample", session_id: "sample", current_revision_id: "sample", actions: [{ id: "sample", label: "fixture", command_id: "artifact.create", input_schema: {}, payload_template: {} }], capability_manifest: { ...manifest, allowed_domain_commands: generatedActionAllowed ? ["artifact.create"] : [] } };
          }
          if (name === "getGeneratedSurfaceRevision") return (outputs.get(id) as { revision?: unknown })?.revision;
          if (name === "readGeneratedSurfaceBundle") return (outputs.get(id) as { bundle?: unknown })?.bundle;
          if (name === "dispatchGeneratedSurfaceCommand") return { result: {} };
          ...
          if (name === "createEvaluationReport") return { run_scores: [] };
    ...
    for (const { definition, handler } of bindings) {
      ...
      for (const inputSource of definition.sources.slice(1)) {
        await handler.execute({ ...context, inputSource, correlationId: `coverage-${definition.id}-${inputSource}` }, input);
      }
      ...
      if (completeInput.success) {
        await handler.execute({ ...context, correlationId: `coverage-${definition.id}-complete` }, completeInput.data);
      }
      for (const [variantIndex, variant] of topLevelVariants(jsonSchemaFor(definition.input, `${definition.id}.input.variants`))) {
        ...
        try {
          await handler.execute({ ...context, correlationId: `coverage-${definition.id}-variant-${variantIndex}` }, parsedVariant.data);
        } catch (error) {
    ...
    expect(bindings).toHaveLength(114);
    expect(portCalls.size).toBe(113);
    ...
    for (reflectionSuggestionType of ["skill", "knowledge_wiki"]) {
      await reflectionBinding.handler.execute(reflectionFixture.context as never, reflectionFixture.input);
    }
    reflectionSuggestionType = "memory";
    reflectionSuggestionType = "skill_patch" as never;
    await expect(reflectionBinding.handler.execute(reflectionFixture.context as never, reflectionFixture.input)).rejects.toBeInstanceOf(Error);
    reflectionSuggestionType = "memory";
    ...
    await memoryArchiveBinding.handler.execute(memoryArchiveFixture.context as never, memoryArchiveFixture.input);
    ...
    for (automationJobKind of ["wiki_reindex", "skill_curator", "memory_review", "learning_evaluation", "resource_translation", "custom_instruction", "daily_digest"]) {
      await automationRunBinding.handler.execute(automationRunFixture.context as never, automationRunFixture.input);
    }
    ...
    for (rollbackScenario of ["missing", "irreversible", "expired", "missing_path", "invalid_content", "root"]) {
      await expect(rollbackBinding.handler.execute(rollbackFixture.context as never, rollbackFixture.input)).rejects.toBeInstanceOf(Error);
    }
    rollbackScenario = "delete";
    await rollbackBinding.handler.execute(rollbackFixture.context as never, rollbackFixture.input);
    ...
    for (externalDispatchScenario of ["dry_run", "failed", "detailed"]) {
      await externalDispatchBinding.handler.execute(externalDispatchFixture.context as never, externalDispatchFixture.input);
    }
    ...
    for (evaluationScenario of ["existing", "missing_run", "no_skills"]) {
      await evaluationBinding.handler.execute(evaluationFixture.context as never, evaluationFixture.input);
    }
    ...
    for (const kind of ["memory", "wiki", "skill", "collection_record"] as const) {
      const ref = { kind, id: "sample", uri: `${kind}/sample`, ...(kind === "collection_record" ? { collection_id: "sample", record_id: "sample" } : {}) };
      await translationBinding.handler.execute(translationFixture.context as never, { ...(translationFixture.input as Record<string, unknown>), source_ref: ref });
    }
    ...
    await expect(generatedActionBinding.handler.execute(generatedActionFixture.context as never, generatedActionFixture.input)).rejects.toBeInstanceOf(Error);
    ...
    const missingArtifactSessionHandler = bindOperationDefinitions(missingArtifactSessionPorts).find((binding) => binding.definition.id === artifactBinding.definition.id)!.handler;
    await expect(missingArtifactSessionHandler.execute({ ...artifactFixture.context, inputSource: "provider_tool_call" } as never, { ...(artifactFixture.input as Record<string, unknown>), session_id: "sample" })).rejects.toBeInstanceOf(Error);
    ...
    for (const { definition } of bindings) {
      ...
      for (const missingMethod of dependencyMethods) {
        ...
        for (const inputSource of definition.sources) {
          try {
            await failingBinding.handler.execute({ ...fixture.context, inputSource } as never, fixture.input);
          } catch {
            guardedFailures += 1;
            break;
          }
