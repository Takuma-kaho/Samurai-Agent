...
interface TranslationChatResult {
  backendRun: { id: string; backend_id: string; room_id?: string };
...
export class TranslationDomainService {
  ...
  async executeJob(job: AutomationJobRecord, session: { id: string }, context: ScheduledAutomationContext): Promise<{
    ...
    if (!chat.backendRun.room_id) throw this.dependencies.requestError("conflict", `resource_translation_backend_run_room_missing:${chat.backendRun.id}`);
    const now = nowIso();
    ...
    const change: NewWorkspaceChangeRecord = { id: createId("change"), run_id: chat.backendRun.id, session_id: session.id, room_id: chat.backendRun.room_id,
