...
import { nowIso, type RoomRecord, type SessionRecord } from "@samurai-agent/core-schemas";
...
describe("Core 06 runtime authorization", () => {
  it("does not let a Workspace Owner read an unjoined Room and revokes Agent execution immediately", async () => {
    ...
    const now = nowIso();
    const room = await store.createRoomWithOwner(roomRecord("room-core06-private", now), humanParticipantId("other-owner"));
    ...
    await store.addRoomMember({ roomId: room.id, participantId: requester, role: "member", actorId: humanParticipantId("other-owner") });
    await store.setRoomAgentPermissions({ roomId: room.id, agentId, canView: true, canEdit: false, canExecute: true, actorId: humanParticipantId("other-owner") });
    ...
    await store.removeRoomAgent({ roomId: room.id, agentId, actorId: humanParticipantId("other-owner") });
  ...
  it("does not create a Session boundary when an unjoined Workspace Owner tries to run it", async () => {
    const store = await createStore();
    const now = nowIso();
    const room = await store.createRoomWithOwner(roomRecord("room-core06-legacy-run", now), humanParticipantId("legacy-run-owner"));
  ...
  it("returns only explicitly shared search candidates and stops returning them after revocation", async () => {
    const store = await createStore();
    const now = nowIso();
    const source = await store.createRoomWithOwner(roomRecord("room-core06-search-source", now), humanParticipantId("source-owner"));
    const target = await store.createRoomWithOwner(roomRecord("room-core06-search-target", now), humanParticipantId("target-owner"));
    ...
    await store.addRoomMember({ roomId: source.id, participantId: member, role: "member", actorId: humanParticipantId("source-owner") });
    await store.addRoomMember({ roomId: target.id, participantId: member, role: "member", actorId: humanParticipantId("target-owner") });
    ...
    const boundary = await store.ensureResourceAccessBoundary({
      ...
      ownerParticipantId: humanParticipantId("source-owner"),
      actorId: humanParticipantId("source-owner")
    });
    ...
    await store.revokeRoomResourceShare({ resourceAccessBoundaryId: boundary.id, targetRoomId: target.id, actorId: member });
    await expect(search.searchMemory(context, "memory", 8)).resolves.toEqual([]);
    await store.close();
  });
  ...
  it("queries only authorized Session candidates before returning search results", async () => {
    ...
    await store.revokeRoomResourceShare({ resourceAccessBoundaryId: boundary.id, targetRoomId: target.id, actorId: sourceOwner });
    await expect(search.searchSessions(context, "secret", 8)).resolves.toEqual([]);
    await store.close();
  });
