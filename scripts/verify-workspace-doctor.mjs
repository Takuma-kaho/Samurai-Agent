...
const temporaryRoot = mkdtempSync(path.join(cacheRoot, "samurai-doctor-"));
const output = path.join(temporaryRoot, "verify.mjs");
const sourceFiles = ["packages/workspace-store/src/index.ts", "scripts/fixtures/workspace-doctor.ts", "scripts/verify-workspace-doctor.mjs", "scripts/lib/core-evidence.mjs"];
try {
  execFileSync(esbuild, [path.join(root, "scripts/fixtures/workspace-doctor.ts"), "--bundle", "--platform=node", "--format=esm", "--external:better-sqlite3", `--outfile=${output}`], { cwd: root, stdio: "inherit" });
  ...
  writeFileSync(path.join(evidenceDir, "C07.json"), `${JSON.stringify({
    schema_version: 1, test_id: "C07", command: "pnpm core:test:doctor", status: "passed",
    ...committedSourceEvidence(root, sourceFiles), started_at: startedAt, completed_at: completedAt,
    assertions: [
      { name: "Doctor detects all corruption fixtures", actual: result.required_detected, expected: 5 },
      { name: "Automatic repair leaves healthy Workspace", actual: result.final_health_ok, expected: true },
      { name: "Automatic repair leaves zero issues", actual: result.final_issue_count, expected: 0 }
    ], result
  }, null, 2)}\n`);
