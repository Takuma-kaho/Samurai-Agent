...
const entrypoints = ["packages/runtime/src/index.ts", "packages/workspace-server/src/index.ts", "apps/server/src/index.ts", "apps/web/src/main.ts"];
...
const vueSources = allSources.filter((file) => file.startsWith("apps/web/src/") && file.endsWith(".vue"));
...
const result = { status: oversizedEntrypoints.length === 0 && missingDirectories.length === 0 && vueSources.length === 0 ? "passed" : "partial", entrypoint_limit: 500, module_line_warning: 1200, entrypoints: entrypoints.map((file) => ({ file, lines: lineCount(file) })), oversized_entrypoints: oversizedEntrypoints, large_modules_advisory: largeModules, missing_directories: missingDirectories, web_runtime: "React NativeApp", vue_sources: vueSources };
if (writeEvidence) {
