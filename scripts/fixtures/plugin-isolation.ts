...
`;
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    crash_failed: crash.status === "failed",
    timeout_failed: timeout.status === "failed",
    memory_limit_failed: memory.status === "failed",
    host_continued: true,
    signature_invalid_rejected: true,
    version_incompatible_rejected: true,
    unsigned_entrypoint_rejected: true,
    core_mutations_before_success: 0,
    tool_surface_same_manifest: true,
...
async function writePlugin(directory: string, id: string, apiVersion: string, entrypoint: string, actionNames: string[], invalidSignature = false) {
  ...
  const manifest: PluginManifest = {
    ...
    ...(directory === "isolation" ? { renderers: [{
      ...
      kind: "custom_view",
      version: "1.0.0",
      title: "Isolation panel",
      description: "Fixture renderer",
      props_schema: { type: "object" },
      category: "custom_view"
    }] } : {}),
    metadata: {
      entrypoint_sha256: createHash("sha256").update(entrypoint).digest("hex"),
  ...
  await writeFile(path.join(pluginDir, "plugin.json"), JSON.stringify({
    ...
    metadata: {
      ...
      plugin_signature: {
        algorithm: "ed25519",
        key_id: "fixture-key",
        signature: invalidSignature ? `${signature.slice(0, -4)}AAAA` : signature
      }
    }
  }, null, 2));
}
