...
async function main(): Promise<void> {
  ...
  assert.equal(expectedOperations.length, 119, "schema_manifest_operation_count_mismatch:119");
  assert.equal(definitions.length, 119, "schema_matrix_definition_count_mismatch:119");
...
function validateManifest(value: StaticSchemaManifest): void {
  ...
  assert.equal(value.review.required_operations, 119, "schema_manifest_required_operation_count_invalid");
  ...
  assert.equal(commands, 102, "schema_manifest_command_count_mismatch");
  assert.equal(queries, 17, "schema_manifest_query_count_mismatch");
  assert.equal(value.review.reviewed_operations, 119, "schema_manifest_reviewed_operation_count_mismatch");
  assert.equal(value.review.unreviewed_operations, 0, "schema_manifest_unreviewed_operations");
  assert.equal(value.review.required_endpoints, 238, "schema_manifest_required_endpoint_count_invalid");
  assert.equal(value.review.reviewed_endpoints, 238, "schema_manifest_reviewed_endpoint_count_invalid");
  assert.equal(value.review.unreviewed_endpoints, 0, "schema_manifest_unreviewed_endpoints");
  assert.equal(value.review.required_schema_nodes, 750, "schema_catalog_required_node_count_invalid");
