...
const backend = (id: string): AgentBackend => ({ id, kind: "external", label: id, sessionPolicy: { acquisition: "none", resume: "unsupported" }, execution_owner: "host", async *runTurn() { yield { event_type: "text_delta", payload: { text: `${id} reply` } }; yield { event_type: "run_completed", terminal_evidence: { kind: "completed", source: "provider_terminal_response" }, payload: { output_summary: `${id} done` } }; } });
...
try {
  ...
  await store.bindAgentBackend({ id: "agent_default", backend_id: selected.id });
  ...
  assert.equal(allowlistAllowed.inbound.status, "blocked");
  assert.equal(allowlistAllowed.inbound.error, "gateway_participant_authentication_required");
  assert.equal(allowlistAllowed.chat, undefined);
  ...
  assert.equal(gateway.inbound.status, "blocked");
  assert.equal(gateway.inbound.error, "gateway_participant_authentication_required");
  assert.equal(gateway.chat, undefined);
  assert.equal(automationBackendRun?.backend_id, selected.id);
  process.stdout.write(`${JSON.stringify({ status: "passed", web_backend: web.backendRun.backend_id, gateway_backend: null, automation_backend: automationBackendRun?.backend_id, consistent: true, unapproved_pairing_blocked: true, allowlist_change_applied_next_input: true, gateway_room_access_blocked: true })}\n`);
} finally {
