...
const expectedCommandCount = Number(process.env.SAMURAI_EXPECT_COMMAND_COUNT ?? "111");
...
assert.equal(operationDefinitions.length, 132);
...
assert.equal(actionCatalogEntries.length, 111);
assert.equal(getDomainCommandCatalogDiagnostics().ok, true);
assert.equal(new Set(operationDefinitions.map((definition) => definition.id)).size, 132);
...
assert.equal(operationFiles.length, 132);
...
assert.equal((indexSource.match(/import operation\d+/g) ?? []).length, 132);
...
assert.equal((binderSource.match(/import operation\d+/g) ?? []).length, 132);
