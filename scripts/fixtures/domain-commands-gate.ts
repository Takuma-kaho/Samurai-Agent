...
const expectedCommandCount = Number(process.env.SAMURAI_EXPECT_COMMAND_COUNT ?? "125");
const expectedQueries = ["agent.list", "agent.view", "browser.extract", "collection.records.list", "collection.schema.docs", "collection.schema.get", "collection.search", "collection.view.present", "curator.snapshot.list", "file.inspect", "file.list", "file.read", "generated_surface.export", "memory.search", "presentation.plan", "room.list", "room.member.list", "room.ownerless.list", "room.resource.share.list", "room.view", "session.search", "skill.search", "skill.view", "wiki.search", "workspace.member.list"];
...
assert.equal(domainQueryEntries.length, 25);
assert.equal(domainLegacyCommandEntries.length, 5);
assert.equal(operationDefinitions.length, 150);
...
assert.equal(actionCatalogEntries.length, 125);
assert.equal(getDomainCommandCatalogDiagnostics().ok, true);
assert.equal(new Set(operationDefinitions.map((definition) => definition.id)).size, 150);
...
assert.equal(operationFiles.length, 150);
...
assert.equal((indexSource.match(/import operation\d+/g) ?? []).length, 150);
...
assert.equal((binderSource.match(/import operation\d+/g) ?? []).length, 150);
