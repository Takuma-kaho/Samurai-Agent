...
try {
  ...
  const event = await store.saveBackendEvent({ id: "e", run_id: "run", session_id: "s", event_type: "text_delta", sequence: 1, payload: { text: `api_key=${secrets[0]} Bearer ${secrets[4]} ${secrets[2]}` }, resource_refs: [], created_at: at });
