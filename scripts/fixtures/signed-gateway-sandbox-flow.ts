...
const backend:AgentBackend={id:"fixture",kind:"samurai_native",label:"fixture",async*runTurn(){yield{event_type:"text_delta",payload:{text:"sandbox reply"}};yield{event_type:"run_completed",payload:{output_summary:"sandbox reply"}}}};
...
try{
  ...
  const inbound=await runtime.handleGatewayInbound({channel:"webhook",source_identity:"signed-source",body:"Run signed sandbox request",backend_id:backend.id,metadata:{message_id:"signed-message",signature_status:"verified",sandbox_backend:"docker"}});assert.equal(inbound.inbound.status,"processed");assert.ok(inbound.session);assert.ok(inbound.chat);
  const delivery=await store.enqueueGatewayDelivery({id:"signed-reply",inbound_id:inbound.inbound.id,session_key:inbound.session!.session_key,channel:"webhook",status:"pending",idempotency_key:"reply:signed-message",payload:{text:"sandbox reply",sandbox_stdout:sandbox.stdout??""},attempt:0,max_attempts:3,created_at:new Date(0).toISOString(),updated_at:new Date(0).toISOString()});
