...
const startServer = async () => { const app = express(); registerBackendEventRoutes(app, store); server = createServer(app); await new Promise<void>((resolve, reject) => { server.once("error", reject); server.listen(socketPath, resolve); }); };
...
try {
  await store.createSession({ id: "event-session", session_key: "web:event:main", title: "Event replay", ui_locale: "en", output_locale: "en", created_at: now, updated_at: now });
  ...
  for (let sequence = 1; sequence <= 100; sequence += 1) {
    await store.saveBackendEvent({
      ...
      payload: { sequence }, resource_refs: [], created_at: new Date(Date.parse(now) + sequence).toISOString()
    });
  }
  await startServer();
