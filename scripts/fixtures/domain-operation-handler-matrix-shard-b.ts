...
function mutationValue(id: string): unknown {
  if (id === "image.edit") return { resource: artifact, revision, operation, activity: [] };
  if (id === "image.generate") return { resource: generatedArtifact, revision, operation, activity: [] };
  return { resource: automationJob, operation, activity: [] };
}
...
async function main(): Promise<void> {
  ...
  for (const testCase of bHandlerExpectations["chat.turn.run"].cases) await runCase("chat.turn.run", testCase, findDefinition("chat.turn.run"), (r) => ({ runChatTurn: asyncRec(r, "runChatTurn", { session, assistant_message: { content: "Fixture reply" }, tool_runs: [], backend_run: null, backend_events: [], memory_candidates: [], skill_candidates: [], artifact_refs: [], collection_refs: [], workspace_changes: [], output_locale: "en", input_locale: "en", surface: null }) }));
