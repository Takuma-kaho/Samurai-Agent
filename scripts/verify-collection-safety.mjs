...
const sourceFiles = ["packages/core-schemas/src/index.ts", "packages/runtime/src/collections/safe-collection.ts", "scripts/fixtures/collection-safety.ts", "scripts/verify-collection-safety.mjs", "scripts/lib/core-evidence.mjs"];
try {
  ...
  writeFileSync(path.join(evidenceDir, "D05.json"), `${JSON.stringify({ schema_version: 1, test_id: "D05", command: "pnpm core:test:collection-safety", status: "passed", ...committedSourceEvidence(root, sourceFiles), started_at: startedAt, completed_at: completedAt, assertions: [
