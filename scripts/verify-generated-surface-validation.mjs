...
const sourceFiles = ["packages/core-schemas/src/index.ts", "packages/action-catalog/src/index.ts", "packages/runtime/src/presentation/generated-surface.ts", "scripts/fixtures/generated-surface-validation.ts", "scripts/verify-generated-surface-validation.mjs", "scripts/lib/core-evidence.mjs", "scripts/lib/verifier-assertions.mjs"];
try {
  ...
  const evidence = { schema_version: 1, test_id: "D02", command: "pnpm core:test:generated-surface", status: verifierEvidenceStatus(result, failures), ...committedSourceEvidence(root, sourceFiles), started_at: startedAt, completed_at: completedAt, assertions, ...(failures.length > 0 ? { failures } : {}), result };
  writeFileSync(path.join(evidenceDir, "D02.json"), `${JSON.stringify(evidence, null, 2)}\n`); process.stdout.write(`${rawResult}\n`);
  reportVerifierFailures("D02", failures);
