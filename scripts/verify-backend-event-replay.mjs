...
const sourceFiles = ["packages/core-schemas/src/index.ts", "packages/workspace-store/src/index.ts", "apps/server/src/index.ts", "apps/server/src/routes/backend-events.ts", "scripts/fixtures/backend-event-replay.ts", "scripts/verify-backend-event-replay.mjs", "scripts/lib/core-evidence.mjs"];
try {
  ...
  writeFileSync(path.join(evidenceDir, "B07.json"), `${JSON.stringify({
    schema_version: 1, test_id: "B07", command: "pnpm core:test:event-replay", status: "passed",
