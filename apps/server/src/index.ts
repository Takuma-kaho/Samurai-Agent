...
export async function createApiServer(options: CreateApiServerOptions = {}): Promise<ApiServer> {
  ...
  app.post("/api/workspace/repair", async (req, res, next) => {
    try {
      res.json(await store.repairWorkspace({ dryRun: req.body?.dry_run !== false }));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.post("/api/workspace/backups", async (_req, res, next) => {
    try {
      res.status(201).json(await store.createWorkspaceBackup());
    } catch (error) {
      next(error);
    }
  });
  ...
  app.post("/api/workspace/backups/:id/restore", async (req, res, next) => {
    try {
      res.json(await store.restoreWorkspaceBackup(req.params.id));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.post("/api/resource-translations", async (req, res, next) => {
    try {
      ...
      res.status(201).json(await store.saveResourceTranslation(record));
    } catch (error) {
      next(error);
    }
  });
  ...
  app.post("/api/resource-translations/jobs", async (req, res, next) => {
    try {
      ...
      res.status(201).json(runtimeWritePayload(await runtime.saveResourceTranslationJob({
        source_ref: sourceRef,
        source_locale: asSupportedLocale(req.body?.source_locale),
        target_locale: targetLocale,
        ...
        max_attempts: typeof req.body?.max_attempts === "number" ? req.body.max_attempts : undefined
      })));
    } catch (error) {
      next(error);
    }
  ...
  app.get("/api/chat/sessions/:sessionId", async (req, res, next) => {
    ...
    app.post("/api/curator/skill-actions/apply", async (req, res, next) => {
      try {
        ...
        res.json(runtimeWritePayload(await runtime.applyCuratorSkillAction({
          skillId,
          action
        })));
      } catch (error) {
    ...
    app.post("/api/gateway/pairing-policies", async (req, res, next) => {
      try {
        ...
        res.status(201).json(await runtime.saveGatewayPairingPolicy(parsed));
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/routing-policies", async (req, res, next) => {
      try {
        ...
        res.status(201).json(await runtime.saveGatewayRoutingPolicy(parsed));
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/pairings/expire", async (_req, res, next) => {
      try {
        res.json(await runtime.expireGatewayPairings());
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/repair", async (req, res, next) => {
      try {
        res.json(await runtime.repairGatewayState({ dryRun: req.body?.dry_run !== false }));
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/pairings/:id/approve", async (req, res, next) => {
      try {
        res.json(await runtime.approveGatewayPairing(req.params.id));
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/pairings/:id/reject", async (req, res, next) => {
      try {
        res.json(await runtime.rejectGatewayPairing(req.params.id));
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/pairings/:id/rotate", async (req, res, next) => {
      try {
        res.json(await runtime.rotateGatewayPairing(req.params.id));
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/pairings/:id/revoke", async (req, res, next) => {
      try {
        res.json(await runtime.revokeGatewayPairing(req.params.id));
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/mcp-configs", async (req, res, next) => {
      try {
        ...
        const saved = await store.saveGatewayMcpConfig(parsed);
    ...
    app.post("/api/gateway/concurrency-locks/expire", async (_req, res, next) => {
      try {
        const expired = await store.expireGatewayConcurrencyLocks();
        res.json({ expired_count: expired.length, locks: expired });
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/sandbox-instances/:id/recreate", async (req, res, next) => {
      try {
        res.json(await runtime.recreateGatewaySandboxInstance(req.params.id));
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/sandbox-instances/:id/delete", async (req, res, next) => {
      try {
        res.json(await runtime.deleteGatewaySandboxInstance(req.params.id));
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/gateway/sandbox-instances/:id/sync", async (req, res, next) => {
      try {
        ...
        res.json(await runtime.syncGatewaySandboxWorkspace(req.params.id, {
          direction: direction ? GatewaySandboxWorkspaceSyncDirectionSchema.parse(direction) : undefined,
          dryRun: req.body?.dry_run !== false
        }));
      } catch (error) {
        next(error);
    ...
    app.post("/api/automation/jobs/:id/requeue", async (req, res, next) => {
      try {
        const job = await store.requeueAutomationJob(req.params.id, {
          nextRunAt: typeof req.body?.next_run_at === "string" ? req.body.next_run_at : undefined
        });
        if (!job) {
          res.status(404).json({ error: "automation_job_not_found" });
          return;
    ...
    app.post("/api/automation/jobs/:id/release-lock", async (req, res, next) => {
      try {
        const job = await store.releaseAutomationJobLock(req.params.id, typeof req.body?.now === "string" ? req.body.now : undefined);
        if (!job) {
          res.status(404).json({ error: "automation_job_not_found" });
          return;
        }
    ...
    app.post("/api/grants", async (req, res, next) => {
      try {
        ...
        const result = await runtime.createGrant({
          capabilityId: typeof req.body?.capability_id === "string" ? req.body.capability_id : undefined,
          operation: req.body.operation,
          actorIdentity,
          channel: typeof req.body?.channel === "string" ? req.body.channel : undefined,
          resourceScope: typeof req.body?.resource_scope === "string" ? req.body.resource_scope : undefined,
          grantedBy: typeof req.body?.granted_by === "string" ? req.body.granted_by : undefined,
          reason: typeof req.body?.reason === "string" ? req.body.reason : undefined,
          expiresAt: typeof req.body?.expires_at === "string" ? req.body.expires_at : undefined
        });
    ...
    app.post("/api/grants/:id/revoke", async (req, res, next) => {
      try {
        const result = await runtime.revokeGrant({
          grantId: req.params.id,
          revokedBy: typeof req.body?.revoked_by === "string" ? req.body.revoked_by : undefined,
          reason: typeof req.body?.reason === "string" ? req.body.reason : undefined
        });
    ...
    app.post("/api/rollback/:id/restore", async (req, res, next) => {
      try {
        res.status(201).json(await runtime.restoreRollbackPoint(req.params.id));
      } catch (error) {
        next(error);
      }
    });
    ...
    app.post("/api/skills/:id/support", async (req, res, next) => {
      try {
        ...
        const result = await runtime.saveSkillSupportFile({
    ...
    app.post("/api/collections/reindex", async (_req, res, next) => {
      try {
        res.json(runtimeWritePayload(await runtime.reindexCollections()));
      } catch (error) {
        next(error);
      }
    });
...
function startAutomationScheduler(runtime: AgentRuntime): AutomationScheduler | undefined {
  ...
  const tick: AutomationScheduler["tick"] = async (now = nowIso()) => {
    ...
    try {
      ...
      state.last_run_count = runs.length;
      state.last_error = undefined;
      return runs;
    } catch (error) {
      state.last_error = error instanceof Error ? redactSecretLikeString(error.message) : redactSecretLikeString(String(error));
