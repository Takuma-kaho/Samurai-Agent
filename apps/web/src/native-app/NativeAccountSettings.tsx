  ...
  const loadPreferences = useCallback(async (): Promise<void> => {
    if (!loadFromStore) {
      ...
      if (!storedPreferencesRef.current && !initialPreferences) {
        setLocalLoadError("本人設定を取得できません。親画面からAccount設定storeを接続してください。");
      }
      return;
    }
  ...
  return <>
    ...
    <div className="native-account-settings">
      <header className="native-account-settings__header">
        <div>
          <span className="native-account-settings__eyebrow">Account settings</span>
          <h1 id="native-account-settings-title">本人設定</h1>
          <p>表示名と回答設定は、このAccountの端末内設定として管理します。</p>
        </div>
        <button className="native-account-settings__back" type="button" onClick={requestBack} disabled={navigationDisabled} aria-label="本人設定を閉じて元の画面へ戻る">戻る</button>
      </header>
      ...
      <div className="native-account-settings__layout">
        <nav className="native-account-settings__nav" aria-label="本人設定カテゴリ">
        ...
        <main className="native-account-settings__detail" aria-labelledby="native-account-settings-title">
          <div className="native-account-settings__status" aria-live="polite">
            {loading ? <p role="status">本人設定を読み込んでいます…</p> : null}
...
.native-account-settings { --nas-ink: var(--native-copy, #edf2eb); --nas-muted: var(--native-muted, #a4afa7); --nas-dim: var(--native-dim, #6f7a73); --nas-line: var(--native-line, rgba(204, 218, 209, .13)); --nas-line-strong: var(--native-line-strong, rgba(204, 218, 209, .25)); --nas-panel: rgba(13, 17, 16, .94); --nas-panel-soft: rgba(27, 34, 30, .68); --nas-accent: var(--native-accent, #f1a65c); --nas-success: var(--native-success, #8ad8b2); --nas-danger: var(--native-danger, #ee8981); background: radial-gradient(circle at 80% 0%, rgba(241, 166, 92, .11), transparent 24rem), linear-gradient(145deg, rgba(22, 29, 26, .98), var(--nas-panel) 48%); box-sizing: border-box; color: var(--nas-ink); display: flex; flex-direction: column; height: 100%; min-height: 0; overflow: hidden; }
.native-account-settings *, .native-account-settings *::before, .native-account-settings *::after { box-sizing: border-box; }
.native-account-settings__header { align-items: flex-start; border-bottom: 1px solid var(--nas-line); display: flex; gap: 20px; justify-content: space-between; padding: 30px 36px 24px; }
.native-account-settings__header h1 { font-family: Georgia, "Times New Roman", serif; font-size: clamp(28px, 4vw, 44px); letter-spacing: -.045em; line-height: 1; margin: 5px 0 9px; }
...
.native-account-settings__eyebrow { color: var(--nas-accent); display: block; font-size: 10px; font-weight: 800; letter-spacing: .16em; text-transform: uppercase; }
.native-account-settings__back { background: transparent; border: 1px solid var(--nas-line-strong); border-radius: 10px; color: var(--nas-muted); cursor: pointer; font: inherit; min-height: 36px; padding: 7px 13px; }
.native-account-settings__back:hover, .native-account-settings__back:focus-visible { color: var(--nas-ink); border-color: var(--nas-accent); }
.native-account-settings__layout { display: grid; grid-template-columns: minmax(180px, 240px) minmax(0, 1fr); min-height: 0; flex: 1; }
.native-account-settings__nav { border-right: 1px solid var(--nas-line); display: grid; align-content: start; gap: 4px; padding: 28px 18px; }
.native-account-settings__nav button { background: transparent; border: 1px solid transparent; border-radius: 10px; color: var(--nas-muted); cursor: pointer; font: inherit; min-height: 42px; padding: 10px 13px; text-align: left; }
.native-account-settings__nav button:hover { background: rgba(255, 255, 255, .045); color: var(--nas-ink); }
.native-account-settings__nav button.is-selected { background: rgba(241, 166, 92, .12); border-color: rgba(241, 166, 92, .3); color: var(--nas-accent); font-weight: 750; }
.native-account-settings__mobile-category { display: none; }
.native-account-settings__detail { min-width: 0; overflow: auto; padding: 30px 36px 60px; }
.native-account-settings__section { margin: 0 auto; max-width: 880px; }
.native-account-settings__section-heading { border-bottom: 1px solid var(--nas-line); margin-bottom: 18px; padding-bottom: 17px; }
.native-account-settings__section-heading h2 { font-family: Georgia, "Times New Roman", serif; font-size: clamp(22px, 3vw, 30px); letter-spacing: -.035em; line-height: 1.05; margin: 6px 0 8px; }
.native-account-settings__card { background: var(--nas-panel-soft); border: 1px solid var(--nas-line); border-radius: 16px; padding: 20px; }
.native-account-settings__field { display: grid; gap: 7px; margin-top: 15px; }
.native-account-settings__field:first-child { margin-top: 0; }
.native-account-settings__field > span, .native-account-settings__readonly-row > span { color: var(--nas-muted); font-size: 12px; font-weight: 720; }
.native-account-settings__field input, .native-account-settings__field select, .native-account-settings__field textarea { background: rgba(0, 0, 0, .22); border: 1px solid var(--nas-line-strong); border-radius: 9px; color: var(--nas-ink); font: inherit; font-size: 13px; min-width: 0; padding: 11px 12px; width: 100%; }
...
.native-account-settings__field textarea { line-height: 1.6; min-height: 190px; resize: vertical; }
...
.native-account-settings__readonly-row { align-items: center; border-top: 1px solid var(--nas-line); display: flex; flex-wrap: wrap; gap: 10px; margin-top: 20px; padding-top: 15px; }
.native-account-settings__readonly-row code { background: rgba(0, 0, 0, .2); border: 1px solid var(--nas-line); border-radius: 7px; color: var(--nas-ink); font-size: 12px; overflow-wrap: anywhere; padding: 7px 9px; }
.native-account-settings__character-count { color: var(--nas-dim); font-size: 11px; margin-top: 6px; text-align: right; }
.native-account-settings__field-error { color: var(--nas-danger); font-size: 11px; line-height: 1.45; margin: 6px 0 0; }
.native-account-settings__actions { display: flex; flex-wrap: wrap; gap: 9px; justify-content: flex-end; margin-top: 18px; }
.native-account-settings__primary-button, .native-account-settings__quiet-button { border-radius: 9px; cursor: pointer; font: inherit; font-size: 12px; font-weight: 720; min-height: 34px; padding: 8px 13px; }
.native-account-settings__primary-button { background: var(--nas-accent); border: 1px solid var(--nas-accent); color: #28170b; }
.native-account-settings__quiet-button { background: rgba(255, 255, 255, .04); border: 1px solid var(--nas-line-strong); color: var(--nas-muted); }
.native-account-settings__danger-button { background: transparent; border: 1px solid rgba(238, 137, 129, .55); border-radius: 9px; color: var(--nas-danger); cursor: pointer; font: inherit; font-size: 12px; font-weight: 720; min-height: 34px; padding: 8px 13px; }
.native-account-settings__primary-button:hover { filter: brightness(1.08); }
.native-account-settings__quiet-button:hover { background: rgba(255, 255, 255, .08); color: var(--nas-ink); }
...
.native-account-settings__feedback { border-radius: 9px; font-size: 12px; line-height: 1.5; margin: 14px 0 0; padding: 10px 12px; }
.native-account-settings__feedback.is-success { background: rgba(138, 216, 178, .1); border: 1px solid rgba(138, 216, 178, .3); color: var(--nas-success); }
...
.native-account-settings__feedback.is-notice { background: rgba(241, 166, 92, .1); border: 1px solid rgba(241, 166, 92, .3); color: var(--nas-accent); }
.native-account-settings__status { color: var(--nas-muted); font-size: 12px; line-height: 1.5; margin: 0 0 16px; }
.native-account-settings__status.is-error { color: var(--nas-danger); }
.native-account-settings__status + .native-account-settings__quiet-button { margin-bottom: 18px; }
.native-account-settings__theme-grid { display: grid; gap: 11px; grid-template-columns: repeat(3, minmax(0, 1fr)); }
.native-account-settings__theme-card { align-items: center; background: var(--nas-panel-soft); border: 1px solid var(--nas-line); border-radius: 14px; color: inherit; cursor: pointer; display: grid; gap: 10px; min-width: 0; padding: 17px; position: relative; text-align: left; }
.native-account-settings__theme-card:hover { border-color: var(--nas-line-strong); background: rgba(255, 255, 255, .06); }
.native-account-settings__theme-card.is-selected { border-color: var(--nas-accent); box-shadow: inset 0 0 0 1px rgba(241, 166, 92, .25); }
.native-account-settings__theme-card strong { display: block; font-size: 13px; }
.native-account-settings__theme-card small { color: var(--nas-muted); display: block; font-size: 11px; line-height: 1.45; margin-top: 5px; }
.native-account-settings__theme-swatch { align-items: center; border: 1px solid var(--nas-line-strong); border-radius: 10px; display: flex; height: 54px; justify-content: center; overflow: hidden; position: relative; width: 100%; }
...
.native-account-settings__theme-swatch::before { height: 62px; left: -10%; opacity: .8; top: -22px; transform: rotate(-15deg); width: 120%; }
.native-account-settings__theme-swatch::after { bottom: 7px; height: 8px; left: 12%; width: 55%; }
.native-account-settings__theme-swatch span { height: 8px; right: 12%; width: 22%; }
.native-account-settings__theme-swatch.is-dark { background: #17211c; }
.native-account-settings__theme-swatch.is-dark::before { background: #2c4136; }
.native-account-settings__theme-swatch.is-dark::after, .native-account-settings__theme-swatch.is-dark span { background: #f1a65c; }
.native-account-settings__theme-swatch.is-light { background: #e9eee8; }
.native-account-settings__theme-swatch.is-light::before { background: #cbd9cf; }
.native-account-settings__theme-swatch.is-light::after, .native-account-settings__theme-swatch.is-light span { background: #8d5a30; }
.native-account-settings__theme-swatch.is-special { background: #202743; }
.native-account-settings__theme-swatch.is-special::before { background: #3a4670; }
.native-account-settings__theme-swatch.is-special::after, .native-account-settings__theme-swatch.is-special span { background: #c9d4ff; }
.native-account-settings__theme-check { color: var(--nas-accent); font-size: 18px; position: absolute; right: 12px; top: 10px; }
.native-account-settings__empty { border: 1px dashed var(--nas-line-strong); border-radius: 12px; color: var(--nas-dim); font-size: 12px; margin: 0; padding: 18px; }
.native-account-settings__connection-list { display: grid; gap: 10px; list-style: none; margin: 0; padding: 0; }
.native-account-settings__connection-item { align-items: flex-start; background: var(--nas-panel-soft); border: 1px solid var(--nas-line); border-radius: 14px; display: grid; gap: 16px; grid-template-columns: minmax(0, 1fr) minmax(155px, auto); padding: 16px; }
.native-account-settings__connection-item > div:first-child { display: grid; gap: 5px; min-width: 0; }
.native-account-settings__connection-item strong { font-size: 14px; }
...
.native-account-settings__connection-state { align-items: flex-end; display: flex; flex-direction: column; gap: 6px; text-align: right; }
.native-account-settings__connection-state > span { border-radius: 999px; font-size: 10px; font-weight: 800; padding: 4px 8px; }
.native-account-settings__connection-state > span.is-synced { background: rgba(138, 216, 178, .12); color: var(--nas-success); }
.native-account-settings__connection-state > span.is-pending { background: rgba(241, 166, 92, .12); color: var(--nas-accent); }
...
.native-account-settings__navigation-prompt { align-items: center; background: rgba(5, 8, 7, .62); display: flex; inset: 0; justify-content: center; padding: 24px; position: fixed; z-index: 20; }
.native-account-settings__navigation-prompt-card { background: var(--nas-panel); border: 1px solid rgba(241, 166, 92, .45); border-radius: 14px; box-shadow: 0 18px 55px rgba(0, 0, 0, .45); max-width: 460px; padding: 22px; width: 100%; }
.native-account-settings__navigation-prompt-card h2 { font-size: 18px; margin: 0; }
.native-account-settings__navigation-prompt-card p { color: var(--nas-muted); font-size: 13px; line-height: 1.6; margin: 12px 0 0; }
@media (max-width: 720px) { .native-account-settings__header { padding: 24px 20px 18px; } .native-account-settings__layout { display: block; } .native-account-settings__nav { display: none; } .native-account-settings__mobile-category { display: grid; gap: 6px; padding: 16px 20px 0; } .native-account-settings__mobile-category label { color: var(--nas-muted); font-size: 11px; font-weight: 700; } .native-account-settings__mobile-category select { background: var(--nas-panel-soft); border: 1px solid var(--nas-line-strong); border-radius: 9px; color: var(--nas-ink); font: inherit; padding: 10px; } .native-account-settings__detail { padding: 24px 20px 45px; } .native-account-settings__theme-grid { grid-template-columns: 1fr; } .native-account-settings__connection-item { grid-template-columns: 1fr; } .native-account-settings__connection-state { align-items: flex-start; text-align: left; } }
`;
